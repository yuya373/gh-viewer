...
(defmethod gh-viewer-issue-open-comment-buffer ((issue gh-issues-issue) repo)
  (cl-labels
      ((create-buffer (comments)
                      (let ((buf (get-buffer-create (gh-viewer-issue-comment-buffer-name issue repo))))
                        (with-current-buffer buf
                          (setq buffer-read-only nil)
                          (erase-buffer)
                          (emojify-mode)
                          (mapc #'(lambda (comment)
                                    ...
                                    (insert "\n\n"))
                                comments)
                          (message "comments: %s" (length comments))
    ...
    (let ((response (gh-issues-comments-list (gh-issues-api :sync nil :cache nil)
                                             ...
                                             (oref issue number))))
      (gh-url-add-response-callback response #'create-buffer))))
